...
def my_speedup_abbr(x):
    ...
    # x is new / legacy, so legacy /new is speedup
    x = 1/x
    # check if it rounds to 1.00 → show "~1.0"
    if round(x, 2) == 1.00:
        prefix = "~"
    

    label = prefix + str(my_round(x, 2))
    return str(label)
...
assert ('legacy' in frmwrks)
frmwrks.remove('legacy')
frmwrks.append('legacy')
lfilter = ['benchmark', 'domain'] + frmwrks

# get improvement over legacy (keep times in best_wide_time for legacy column), reorder columns
...
for f in frmwrks:
    best_wide[f] = best_wide[f] / best_wide_time['legacy']
...
for j in range(len(overall_wide.columns)):
    if j < len(overall_wide.columns) - 1:
        ...
        label = overall_time_wide['legacy'].to_numpy()[0]
...
for i in range(len(best_wide['benchmark'])):
    # annotate with improvement over legacy
    for j in range(len(hm_data.columns)):
        ...
        if j < len(hm_data.columns) - 1:
            ...
            label = best_wide_time['legacy'].to_numpy()[i]
            p = cidata[(cidata['framework_'] == f)
                       & (cidata['benchmark_'] == b)]['perc']
            try:
                ci = int(p.to_numpy()[0])
